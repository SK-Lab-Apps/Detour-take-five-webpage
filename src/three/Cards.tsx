...
const vert = /* glsl */ `
  ...
  uniform float uSettle;   // menu(0) → "put the phone down" calm motes(1)
  ...
  attribute vec3  aVoid;   // final resting spot for the surviving motes
  ...
  attribute float aMote;   // 1 = survives into the calm ending as a warm mote
  ...
  varying float vMote;     // aMote * uSettle → how "mote" this fragment is right now
  ...
  void main(){
    ...
    // "put the phone down": surviving motes drift to a sparse, slow-breathing resting field.
    float mote = aMote * uSettle;
    vec3 voidPos = aVoid + uMotion * vec3(
      sin(uTime * 0.25 + aSeed * 5.0) * 0.5,
      sin(uTime * 0.20 + aSeed * 8.0) * 0.6,
      cos(uTime * 0.18 + aSeed * 4.0) * 0.4
    );
    center = mix(center, voidPos, mote);
    ...
    // size & aspect: little portrait PHONES in the swarm → calm squarish tokens in the menu
    float swarmSize = uSize * (0.5 + 0.5 * aSeed) * 0.82;
    float menuSize  = uSize * mix(0.42, 1.0, aHero) * (0.9 + 0.2 * aSeed);
    float size = mix(swarmSize, menuSize, g);
    // in the ending, non-motes shrink away to nothing; motes swell into soft round glints
    size *= mix(1.0, aMote * 1.5, uSettle);
    float W = size * mix(0.60, 1.02, g); // narrow phone → squarish token
    float H = size * mix(1.30, 1.02, g); // tall phone → squarish token
    ...
    vRadius = min(vHalf.x, vHalf.y) * mix(0.30, 0.95, g);  // rounded phone → pill token
...
const frag = /* glsl */ `
  ...
  void main(){
    ...
    // phone read in the feed: a brighter inner "screen" panel inside a thin bezel, faded out
    // as the cards calm into menu tokens.
    float phone = 1.0 - vGather;
    vec2 sHalf = vHalf * vec2(0.80, 0.82);
    float sRad = min(sHalf.x, sHalf.y) * 0.40;
    float dS = length(max(abs(vP) - (sHalf - sRad), 0.0)) - sRad;
    float inScreen = 1.0 - smoothstep(0.0, aa, dS);
    vec3 lit = min(col + vec3(0.16, 0.17, 0.20), vec3(1.0)); // cool, lit screen
    col = mix(col, mix(col, lit, inScreen), phone);

    // ending: surviving motes glow a soft, warm cream and read mostly as their gentle halo
...
export function Cards({
  ...
  const geo = useMemo(() => {
    ...
    const voidArr = new Float32Array(count * 3) // resting spots for the surviving motes
    const seed = new Float32Array(count)
    const tier = new Float32Array(count)
    const hero = new Float32Array(count)
    const mote = new Float32Array(count)
    ...
    for (let i = 0; i < count; i++) {
      ...
      // ~1 in 40 cards survives the ending as a sparse, slow-breathing warm mote.
      if (i % 40 === 0) {
        mote[i] = 1
        voidArr[i * 3] = (Math.random() * 2 - 1) * 7.0
        voidArr[i * 3 + 1] = (Math.random() * 2 - 1) * 4.2
        voidArr[i * 3 + 2] = -1.0 + (Math.random() * 2 - 1) * 3.0
      }
    ...
    g.setAttribute('aVoid', new THREE.InstancedBufferAttribute(voidArr, 3))
    ...
    g.setAttribute('aMote', new THREE.InstancedBufferAttribute(mote, 1))
  ...
  useFrame((_, dt) => {
    ...
    u.uSettle.value += (journey.settle - u.uSettle.value) * k
